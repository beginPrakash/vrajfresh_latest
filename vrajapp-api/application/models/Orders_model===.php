...
class Orders_model extends CI_Model
{
    ...
    {
        $query = $this->db->select('o.order_id,o.user_id,op.product_id,o.order_total_amount,o.order_datetime,o.order_status,o.is_active')
    ...
    {
        $query = $this->db->select('o.*,s.state,c.country_name as shipping_country')
    ...
    {
        $query = $this->db->select('op.qty,op.old_qty,op.product_name,op.total_amount,op.unit_price,v.product_variant_size')
    ...
    {
        $query = $this->db->select('op.qty,op.old_qty,op.product_name,op.total_amount,op.unit_price,pi.image,v.product_variant_size')
            ->join('tbl_products p', 'op.product_id=p.product_id')
            ->join('tblproduct_variant v', 'op.product_variant_id =v.id', 'left')
    ...
    {
        $query = $this->db->select('op.qty,p.product_name,op.total_amount,op.unit_price,pi.image,v.product_variant_size')
            ->join('tbl_products p', 'op.product_id=p.product_id')
            ->join('tblproduct_variant v', 'op.product_variant_id =v.id', 'left')
            ->join('tbl_product_images pi', 'p.product_id=pi.product_id', 'left')
    ...
    {
        $query = $this->db->select('op.qty,p.product_name,op.total_amount,op.unit_price,v.product_variant_size')
            ->join('tbl_products p', 'op.product_id=p.product_id')
            ->join('tblproduct_variant v', 'op.product_variant_id =v.id', 'left')
            ->where('op.order_id', $data)
            ->where('op.is_active', '1')
