...
class Categories_model extends CI_Model
{
    ...
    public function get_category_by_ids($data)
    {
        if(!empty($data)){
            $query = $this->db->select('category_id,category_name')
                ->where_in('category_id', $data)
                ->where('is_deleted', '0')
                ->get('tbl_categories');
            return $query->result();
        }else{
    ...
    public function get_filter_products_pl($data,$limit, $offset)
    
    {
        // print_r($data);exit;

        $query = $this->db->from('tbl_products p')
            ->select('p.product_id,p.product_name,p.product_slug,p.product_price,p.sale_price,p.product_weight_gms,p.product_image,p.is_perisible_products,p.product_tax,pv.is_out_of_stock as variant_is_out_of_stock, p.is_out_of_stock, pv.id,pv.product_variant_size,pv.variant_price,pv.variant_image')
            ->join('tbl_categories_products_mapping cpm', 'p.product_id = cpm.product_id')
            ->join('tbl_categories c', 'c.category_id=cpm.category_id')
            ->join('tblproduct_variant pv', 'p.product_id=pv.product_id', "LEFT")
            ->join('tbl_brands b', 'p.brand_id = b.brand_id')
     ...
     public function get_filter_products_pl_count($data)
    {
        // print_r($data);exit;
