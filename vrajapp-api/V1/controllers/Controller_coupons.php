			...
			if($json_obj->coupon_code!='')
			{
				...
				if(count($result) > 0)
				{
						...
						if($start_date <= $today && $end_date >= $today)
						{
							...
							#---------------------CHECK PROMO TYPE
							if($result[0]->promotional_type=='M') // multiple use
							{
								$applied_flag = 0;
							}
							else if($result[0]->promotional_type=='S') //single
							{
							...
							else if($result[0]->promotional_type=='OT') //one time
							{
								#check current customer eligible or not for this coupon code
								if($customer_id>0)
								{
							...
							#---------------------CHECK APPLY TO
							if($result[0]->apply_to=='AC') // for all customer
							{
								$applied_flag = 0;
							}
							elseif($result[0]->apply_to=='SG') // for specific customer
							{
								if($customer_id>0)
								{
							...
							if($applied_flag==0)
							{
								if($result[0]->discount_type == "$")
								{
									...
									$ArrData=array('discount_amount' =>  number_format($discount_amount,2),'order_amount'=>number_format($total_order_amount,2));
									$success_message = 'Congratulations! Coupon has been applied successfully';
								}
								else
								{
									$discount_amount =  $total_applicapble_order_amount*((float)$result[0]->discount_value/100); 
									if($result[0]->maximum_order_discount>0 && $discount_amount>$result[0]->maximum_order_discount)
									{
										$discount_amount = $result[0]->maximum_order_discount;
									}  
