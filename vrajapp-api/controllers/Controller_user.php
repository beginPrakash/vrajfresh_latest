...
class Controller_user extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->db->where('tbl_users.created_datetime >=',@$_REQUEST['txtSearchFrom']);		 
			$this->db->where('tbl_users.created_datetime <=',@$_REQUEST['txtSearchTo']);		 
		}
		...
		if(@$_REQUEST['columns'] != "") 
		{
			...
			foreach($rResult->result_array() as $aRow)
			{
               ...
               /*  $actions .= '<input type="checkbox" name="delete[]" class="chkDelete" value="'.$user_id.'" />';
				...
				$row[] = mb_strimwidth(trim($aRow['address']), 0, 10, '...');
				$row[] = $aRow['city'];
				$row[] = $aRow['state'];
				$row[] = $aRow['phone'];
				$row[] = $aRow['email'];
	...
	public function save()
	{
		...
		if(isset($_POST) && $_POST['save_user']!="" && $_POST['save_user']=="Add")
		{
			...
			$address1 = (trim($_POST['address1']))?trim($_POST['address1']):null;
	...
	public function profile()
	{
		...
		if($this->input->post('submit'))
		{
			...
			global $ip_Dtl;
			$ip_Dtl =  get_user_location_details();//call function in common helper
			//echo "from=".$ip_Dtl['geoplugin_status'] ."<br>";
			$ip_country = $ip_Dtl['geoplugin_countryName'];
			$ip_region=$ip_Dtl['geoplugin_region'];
			$ip_city = $ip_Dtl['geoplugin_city'];
			$ip_postalcode = $ip_Dtl['geoplugin_areaCode'];
			$ip_latitude = $ip_Dtl['geoplugin_latitude'];
			$ip_longitude = $ip_Dtl['geoplugin_longitude'];
				$time_zone = get_time_zone($ip_Dtl['geoplugin_countryCode'],$ip_Dtl['geoplugin_regionCode']);
		 */
		...
		else
		{
			...
			if($result){
				...
				$this->session->set_userdata($data);
					...
					$_SESSION['user_redirect_from_login'] = 'yes';
					$redirecturl = '';
					if(isset($_SESSION['last_visited_url_by_qeuser']) && $_SESSION['last_visited_url_by_qeuser']!='')
					{
						$redirecturl = urldecode($_SESSION['last_visited_url_by_qeuser']);
					}
					if (strpos($redirecturl, "controller_job/editOrder") === FALSE) {
					}
					else
					{
						if($redirecturl!='')
						{
							redirect('admin/controller_job/assigned_job/job');
						}
					}
					if($redirecturl ==""){
						redirect('home');
