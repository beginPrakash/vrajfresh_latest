		...
		<div class="container-flex ">
			...
			<div class="product-filter">
				...
				<div class="product-grid" id="category_product">
					<input type="hidden" id="tag_id" value="<?php echo ($id != "" ? $id : ""); ?>">
...
<script>
	...
	$(document).ready(function() {
		...
		get_tag_products(api_url_prefix);
	...
	function get_tag_products(api_url_prefix) {
		var id = $("#tag_id").val();
		...
		if (search != "" && search != undefined) {
			...
			$.ajax({
				"type": "POST",
				"url": api_url_prefix + 'get-tag-product-search',
				"data": JSON.stringify(json_request),
				"dataType": "JSON",
				"success": function(response) {
					if (response.data.product_id != null) {
						...
						for (let a = 0; a < response.data.products.length; a++) {
							...
							first_variant_out_of_stock = false;
							...
							if (response.data.products[a].product_size.length > 1) {
								...
								var price_weight = '<form name="form" id="form"><select name="jumpMenu" class="variants' + out_of_stock_variant + '" id="jumpMenu">';
								for (let j = 0; j < response.data.products[a].product_size.length; j++) {
									price_weight = price_weight.concat('<option value="' + response.data.products[a].product_size[j].size + '-' + response.data.products[a].product_size[j].price + '-' + response.data.products[a].product_size[j].variant_id + out_of_stock_variant + '">' + response.data.products[a].product_size[j].size + 'gms - $' + response.data.products[a].product_size[j].price + '</option>');
								}
								price_weight += '</select></form>';
							} else {
								var price_weight = '<span>' + response.data.products[a]
									.product_weight_gms + 'gms</span> - <strong>$' + response.data.products[a]
			...
			$.ajax({
				"type": "POST",
				"url": api_url_prefix + 'get-product-by-tag-id',
				"data": JSON.stringify(json_request),
				"dataType": "JSON",
				"success": function(response) {
					if (response.data != null) {

						product_title = '<h2>' + response.data.tag.tag + '</h2>';
						if (response.data.product_id != null) {
							var first_variant_out_of_stock = false;
							for (let a = 0; a < response.data.product_id.length; a++) {
								...
								if (response.data.products[a].product_size.length > 1) {
									...
									var price_weight = '<form name="form" id="form"><select name="jumpMenu" class="variants' + out_of_stock_variant + '" id="jumpMenu">';
									for (let j = 0; j < response.data.products[a].product_size.length; j++) {
										price_weight = price_weight.concat('<option value="' + response.data.products[a].product_size[j].size + '-' + response.data.products[a].product_size[j].price + '-' + response.data.products[a].product_size[j].variant_id + '">' + response.data.products[a].product_size[j].size + 'gms - $' + response.data.products[a].product_size[j].price + '</option>');
									}
									price_weight += '</select></form>';
								} else {
									var price_weight = '<span>' + response.data.products[a]
						...
						for (let a = 0; a < response.data.product_details.length; a++) {
							if (response.data.product_details[a].product_size.length > 1) {
								...
								for (let j = 0; j < response.data.product_details[a].product_size.length; j++) {
									price_weight = price_weight.concat('<option value="' + response.data.product_details[a].product_size[j].size + '-' + response.data.product_details[a].product_size[j].price + '-' + response.data.product_details[a].product_size[j].variant_id + '">' + response.data.product_details[a].product_size[j].size + 'gms - $' + response.data.product_details[a].product_size[j].price + '</option>');
								}
								price_weight += '</select></form>';
							} else {
								var price_weight = '<span>' + response.data.product_details[a]
									.product_weight_gms + 'gms</span> - <strong>$' + response.data.product_details[a]
									.product_price + '</strong>';
							}
							var out_of_stock = "<div class='product-stock-message'></div>";
							var out_of_stock_class = "";
