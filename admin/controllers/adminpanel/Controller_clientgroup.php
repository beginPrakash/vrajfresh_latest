		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
			...
			$ArrCustomersFlag = false;
					...
					if( (isset($_POST['number_of_order_from']) && $_POST['number_of_order_from']!='') || (isset($_POST['number_of_order_to']) && $_POST['number_of_order_to']!='') )
					{
						...
						$qry = "SELECT u.user_id, COUNT( b.customer_id ) ordercnt FROM tbl_users u LEFT JOIN tbl_orders b ON u.user_id = b.customer_id WHERE 1=1".$condition;																		
					...
					if(isset($_POST['no_order_in_last_days']) && $_POST['no_order_in_last_days']!='')
					{
						...
						*/
						...
						$qry = "select u.user_id FROM tbl_users u WHERE u.user_id NOT IN (SELECT customer_id FROM tbl_orders WHERE created_date>=DATE(NOW()) - INTERVAL ".($no_order_in_last_days-1)." DAY)";
