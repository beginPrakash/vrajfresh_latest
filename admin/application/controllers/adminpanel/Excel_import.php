...
class Excel_import extends CI_Controller
{
	...
	function import()
	{
		if (isset($_FILES["file"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if ($product_count == 0 && $product_count_sku == 0) {
						...
						/* ADD MAIN PRODUCT AS VARIANT IF MAIN PRODUCT V START */
						if (strtolower($product_type) == 'variant') {
							...
							if ($product_variant_count == 0) {
								...
								if ($image != "") {
									...
									$ext = explode("?", $ext);
									$ext = $ext[0];
									file_put_contents(PRODUCT_IMAGE_PATH . "uploads/products/" . $guid . "." . $ext, file_get_contents(trim($image)));
									compressImage(SITE_URL . "uploads/products/" . $guid . "." . $ext, SITE_URL . "uploads/products/" . $guid . "." . $ext, 0);

									$product_variant_data['variant_image'] = trim($guid . '.' . $ext);
								} else {
									...
									$ext = explode("?", $ext);
									$ext = $ext[0];
									file_put_contents(PRODUCT_IMAGE_PATH . "uploads/products/" . $guid . "." . $ext, file_get_contents(trim($ArrImage[0])));
									compressImage(SITE_URL . "uploads/products/" . $guid . "." . $ext, SITE_URL . "uploads/products/" . $guid . "." . $ext, 0);

									$product_variant_data['variant_image'] = trim($guid . '.' . $ext);
								}
								/* UPLOAD VARIANT IMAGE END */

								$variant_id = $this->product_model->add_variants($product_variant_data);
								$variant_product_insert_count++;
							}
						}
						/* ADD MAIN PRODUCT AS VARIANT IF MAIN PRODUCT V END */

						$product_insert_count++;
					}


					/* -----------------------UPDATE IMAGE CATEGORY, TAG and IMAGE OF THE PRODUCT START-------------------------------------- */
					if ($product_id > 0) {
						if ($other_image != "") {
							...
							if (is_array($ArrImage) && count($ArrImage) > 0) {
								foreach ($ArrImage as $images) {
									...
									$ext = explode("?", $ext);
									$ext = $ext[0];
									file_put_contents(PRODUCT_IMAGE_PATH . "uploads/products/" . $guid . "." . $ext, file_get_contents(trim($images)));
									compressImage(SITE_URL . "uploads/products/" . $guid . "." . $ext, SITE_URL . "uploads/products/" . $guid . "." . $ext, 0);
								...
								$ext = explode("?", $ext);
								$ext = $ext[0];
								file_put_contents(PRODUCT_IMAGE_PATH . "uploads/products/" . $guid . "." . $ext, file_get_contents(trim($other_image)));
								compressImage(SITE_URL . "uploads/products/" . $guid . "." . $ext, SITE_URL . "uploads/products/" . $guid . "." . $ext, 0);
								$product_image_data = array(
									'product_id' => trim($product_id),
									'image' => trim($guid . '.' . $ext),
					...
					/* -----------------------UPDATE IMAGE CATEGORY, TAG and IMAGE OF THE PRODUCT END-------------------------------------- */
					...
					if (strtolower($product_type) == 'variant') {
						...
						if (trim($old_product_name) != trim($product_name)) {
							$get_product_id_for_variant = $this->product_model->getProductBySKUgetID(trim($product_sku));
							$product_id_for_variant = $get_product_id_for_variant[0]['product_id'];
							if ($product_variant_count == 0) {
								$product_variant_data = array(
									'product_id' => trim($product_id_for_variant),
									'product_variant_size' => trim($weight),
			...
			if ($product_already_there != '<br><b>The below products has not imported:</b>') {
