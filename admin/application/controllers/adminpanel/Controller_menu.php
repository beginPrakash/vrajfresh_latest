...

{
...

	{
		...
		// You can customize this method to show an "Unauthorized" page or message

		echo "You don't have access to this module. Please go back to the page";

		exit; // Make sure to exit the script after showing the unauthorized page
...

	{
		...
		$ArrData['categories'] = $this->category_model->getCategoryQueryString('', array('is_deleted' => '0', 'is_active' => '1'));
...

	{
		...
		if (isset($_POST) && $_POST['save_menu'] != "" && $_POST['save_menu'] == "Add") {
			...
			if ($menu_id > 0) {
				...
				$ArrMenuCategory = $_POST['menu_category'];

				$ArrAllSubMenuLink = $_POST['sub_menu_link'];

				$ArrAllSubMenuTitle = $_POST['sub_menu_title'];

				$ArrAllSubMenuCategory = $_POST['sub_menu_category'];
				...
				foreach ($ArrMenuTitle as $i => $data) {
					...
					if (isset($ArrMenuTitle[$i][0]) && isset($ArrMenuLink[$i][0])) {
						...
						$menu_category = $ArrMenuCategory[$i][0];
						...
						if (isset($ArrAllSubMenuTitle[$i]) && $parent_menu_item_id > 0) {
							...
							$ArrSubMenuCategory = $ArrAllSubMenuCategory[$i];
							...
							if (is_array($ArrSubMenuTitle) && count($ArrSubMenuTitle) > 0) {
								...
								for ($m = 0; $m < count($ArrSubMenuTitle); $m++) {
									...
									$Arrdata = array(
										...
										'category_id' => $ArrSubMenuCategory[$m],
				...
				redirect($this->module_name . '-update/' . $menu_id);
		...
		if (isset($_POST) && $_POST['save_menu'] != "" && $_POST['save_menu'] == "Update" && $_POST['menu_id'] > 0) {
			...
			if (isset($_POST['sub_menu_category']))
...

	{
		...
		//echo '<pre>';print_r($ArrMenuData['ArrFieldData']);exit;

		$ArrMenuData['ArrMenuItemsData'] = $this->menu_master_model->getMenuItemsUsingMenuId($menu_id);
		//echo '<pre>';print_r($ArrMenuData['ArrMenuItemsData']);exit;
