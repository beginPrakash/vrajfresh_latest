...

{
...

	{
		...
		$column_order = array('null', 'tblblog_master.created_datetime', 'tblblog_master.blog_title', 'tblblog_master.blog_slug', 'tblblog_master.is_active');



		$column_search = array('tblblog_master.blog_id', 'tblblog_master.created_datetime', 'tblblog_master.blog_title', 'tblblog_master.blog_slug', 'tblblog_master.is_active', 'tblblog_master.blog_description', 'tblblog_master.meta_title', 'tblblog_master.meta_descriptions');



		$aColumns = array('tblblog_master.blog_id', 'tblblog_master.created_datetime', 'tblblog_master.blog_title', 'tblblog_master.blog_slug', 'tblblog_master.is_active');
...

	{
		...
		$ArrBlogData['category'] = $this->category_model->category_list_data();
		if ($id > 0) {
...

	{
		...
		if (isset($_POST) && $_POST['save_blog'] != "" && $_POST['save_blog'] == "Add") {
			...
			$blog_slug = (trim($_POST['blog_slug'])) ? $_POST['blog_slug'] : '';

			$category_id = (trim($_POST['category_id'])) ? $_POST['category_id'] : '';
			...
			$ArrBlogData = $this->blog_master_model->getBlogResultBySlug($blog_slug);
			...
			if (count($ArrBlogData) > 0) {
				...
				if ($blog_id > 0) {
					...
					redirect($this->module_name . '-update/' . $blog_id);
		...
		if (isset($_POST) && $_POST['save_blog'] != "" && $_POST['save_blog'] == "Update" && $_POST['blog_id'] > 0) {
			...
			$ArrBlogData = $this->blog_master_model->isUrlExist($blog_slug, $blog_id);
...

	{
		...
		$result = $this->blog_master_model->isUrlExist($blog_slug, $blog_id);
