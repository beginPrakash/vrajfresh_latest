   ...
   <div class="col-sm-12">
    <div class="login_page_detail">
	  ...
	  <h3>Login as a Admin User</h3>
		...
		<?php } ?>
		<?php if (isset($error)) { ?>
		<div class="alert alert-danger" role="alert" style=""><?php echo $error; ?></div>
		<?php } ?>
        <form action="<?php echo base_url() ?>login-process" method="post">
          <div class="form-group user_icon">
            <input type="text" class="form-control" name="login_view" id="login_view" value="<?php if(isset($_COOKIE["username"])) { echo $_COOKIE["username"]; } ?>" placeholder="Username">            
          </div>
          <div class="form-group pwd_icon">
            <input type="password"class="form-control" name="password" id="password" value="<?php if(isset($_COOKIE["password"])) { echo base64_decode( $_COOKIE["password"]); } ?>" placeholder="Password">            
          </div>
          <div class="row">
            <div class="col-xs-12">
