	...
	<div class="col-xs-12">
		<div class="box">
			<div class="box-header filter_div">
				<form class="filter_form" id="frmListDataFilter" action="<?php echo SITE_URL ?>blog" method="post">
					<div class="row">
						<div class="col-sm-2">
						...
						<div class="col-sm-2">
							...
							<input type="hidden" value="<?php echo SITE_URL ?>adminpanel/controller_blog/blog_list_export" id="searchExportaction">
						</div>
						<!-- add export btn - END //- -->
					</div>
				</form>
...
function blog_detail(blog_id)
{
	...
	$.ajax({
		type        : "POST",
		url         : "<?php echo base_url() ?>adminpanel/controller_blog/ajaxShowTblblogMasterData",
		data        : "blog_id="+blog_id,
		success: function(results) {
			$(document.body).css({'cursor' : 'default'});
			$('#recordDetailsPopUp').modal('show');
			$('#detailsPopUpData').html(results);
			$('#detailsPopUpTitle').html('Blog Details');
		},
		error: function() {
			$(document.body).css({'cursor' : 'default'});
...
//- export csv file of blog list - START
...
//- export csv file of blog list - END
