<?php
class catreport_model extends CI_Model
{
	...
	public function getOrderListData($Arr)
	{
        ...
        if(!empty(@$_REQUEST['txtSearchFrom']) || !empty(@$_REQUEST['txtSearchTo']) 
            || !empty(@$_REQUEST['src_category'])){
		    $column_order = array(null,'tbl_categories.category_name','tbl_orders.order_id', 'tbl_categories.category_name', 'tbl_orders.order_datetime','tbl_categories_products_mapping.category_id','tbl_order_products.product_name', 'tbl_order_products.product_id', 'tbl_order_products.qty','tbl_order_products.total_amount','tbl_orders.order_total_amount', 'tbl_orders.order_status', 'tbl_orders.is_active', 'tbl_orders.created_datetime');

            $aColumns = array('tbl_categories.category_name','sum(tbl_order_products.qty) as pro_qty','sum(tbl_order_products.total_amount) as pro_total_amount','tbl_orders.order_id', 'tbl_orders.order_datetime','tbl_categories_products_mapping.category_id','tbl_categories.category_name', 'tbl_order_products.product_name','tbl_order_products.product_id', 'tbl_order_products.qty','tbl_order_products.total_amount', 'tbl_orders.order_total_amount', 'tbl_orders.order_status', 'tbl_orders.is_active', 'tbl_orders.created_datetime');
            ...
            if (@$_REQUEST['src_category'] != "") {
                $this->db->where_in('tbl_categories_products_mapping.category_id', @$_REQUEST['src_category']);
            }
            ...
            if (@$_REQUEST['columns'] != "") {
                ...
                if (isset($_POST['order'])) { // here order processing
                    ...
                    $this->db->order_by('tbl_categories.category_id', 'asc');
                }
