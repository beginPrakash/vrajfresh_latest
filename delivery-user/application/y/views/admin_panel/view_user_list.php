   ...
   <div class="col-xs-12">
      <div class="box">
         <div class="box-header filter_div">
            <!-- Fillter area -->
            <div class="row">
				<form id="frmListDataFilter" action="<?php echo SITE_URL ?>user" method="post">
					  <div class="col-sm-2">
					  ...
					  <div class="col-sm-2">
						 <input type="text" class="form-control date_picker_bottom_center" name="txtSearchFrom" id="txtSearchFrom" placeholder="Created From" title="Created From" autocomplete="false">
					  </div>
					  <div class="col-sm-2">
						 <input type="text" class="form-control date_picker_bottom_center" name="txtSearchTo" id="txtSearchTo"  placeholder="Created To" title="Created To" autocomplete="false">
					  </div>
					  <div class="col-sm-2">
						 <?php
							$options = array('' => 'User Role');
							//$options = "";
							foreach ($usertype as $UserTypeData){
								$options[$UserTypeData['user_role_id']] = $UserTypeData['user_role_name'];
							}
							echo form_dropdown('user_role_id', $options, @$_REQUEST['user_role_id'], 'class="form-control select2" id="user_role_id"');
							?>
					  </div>
				...
				<input type="hidden" value="<?php echo SITE_URL ?>adminpanel/controller_user/user_list_export" id="searchExportaction">
				</div>-->
            </div>
            <!-- Fillter area end-->
         </div>
            ...
            <table id="tblListData" class="display nowrap" cellspacing="0" width="100%">
               <thead>
                  <tr>
			   	...
			   	<tfoot>
					<tr>
						<td colspan="2"><input type="submit" name = "submitForm" value = "Delete" class = "btn btn-danger pull-left" onClick="return delete_multiple_record();" /> </td>
					</tr>
				</tfoot>
            </table>
         </div>
         <input type="hidden" value="<?php echo base_url() ?>adminpanel/controller_user/delete_multiple_user_record" id="multiple_delete_action">
         <input type="hidden" value="<?php echo base_url() ?>adminpanel/controller_user/view_portfolio_ajax" id="view_action">
         <!-- /.box-body -->
      </div>
      <!-- /.box -->
   </div>
...
<script type="text/javascript">
   /* DATA TABLE JS */
   var orderCol = [[ 1, "desc" ]];  
   var cols = [{ "bSortable": false },{ "bSortable": true },{ "bSortable": true },{ "bSortable": true }, { "bSortable": true }, { "bSortable": false }, { "bSortable": true }, { "bSortable": false }, { "bSortable": false }, { "bSortable": false }];
   var colDef = [0,9];
   var aoColumnDefs = [];
...
function user_detail(user_id)
{
	...
	$.ajax({
		type        : "POST",
		url         : "<?php echo base_url() ?>adminpanel/controller_user/ajaxShowTblUserMasterData",
