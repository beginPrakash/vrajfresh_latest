               ...
               <tbody>
                  ...
                  foreach ($ArrOrderProduct as $arr) {
                     ...
                     <tr>
                        ...
                        <td>
                           <input type="text" placeholder="QTY" class="form-control qty"
                        ...
                        <td>
                           <input type="text" placeholder="Product Tax" class="form-control"
                              id="product_tax<?php echo $arr['order_product_id']; ?>"
                              name="product_tax[<?php echo $arr['order_product_id']; ?>]"
                              value="<?php echo $arr['product_tax_amount']; ?>" required readonly>
                        </td>
...
<?php if($ArrFieldData['is_revert'] == 1 || $ArrFieldData['is_capture'] == 1){ ?>
   <form class="form-horizontal" action='<?php echo base_url('order/diffrence/payment/'.$ArrFieldData['order_id']); ?>' id="diffpaymentFrm" role="form" enctype="multipart/form-data" method="post" accept-charset="utf-8">
      ...
      <div class="row">         
         <div class="col-sm-4">
            <input hidden="diffamount" name="diffamount" id="diffamount" value="<?php echo $ArrFieldData['DiffAmount']; ?>" />
            
            <?php if($ArrFieldData['is_capture'] == 1){ ?>
               <input hidden="difftype" id="difftype" name="difftype" value="capture" />
            <button type="button" id="orderdiffBtn" class="btn btn-success">Capture Payment - $ <?php echo $ArrFieldData['DiffAmount']; ?></button>
            <?php } else { ?>
               <input hidden="difftype" id="difftype" name="difftype" value="revert" />
               <button type="button" id="orderdiffBtn" class="btn btn-success">Refund Payment - $ <?php echo $ArrFieldData['DiffAmount']; ?></button>
            <?php }?>
         </div>
      </div>
   </form>
...
<script>
   ...
   function SetPrice(val, product_counter) {
      ...
      if (product_tax == 0) {
         ...
         $("#product_tax" + product_counter).val(state_tax_percentage);
      } else {
         $("#product_tax" + product_counter).val(0);
      }
	  console.log(ArrProductDetails[2]);
